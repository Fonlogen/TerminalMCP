...
async function main() {
  ...
  try {
    ...
    check('/health lists the full toolset', r.json?.tools?.length === 26, JSON.stringify(r.json?.tools));
    ...
    check('tools/list over HTTP works', r.json?.result?.tools?.length === 26, r.text.slice(0, 150));
